...
import { getTransactions, putTransactionSell, getProductsStocks } from "../../Redux/actions";
...
const TablaTransactions = ({ filters }) => {
  ...
  const updateTransaction = (transaction) => {
    ...
    newTransaction["has_swap"] = transaction.swap_products.length > 0 ? true : false;
            ...
            {transactions.map((prod) => (
              <TableRow sx={{ '&:hover': { backgroundColor: 'rgba(0, 0, 0, 0.1)' } }} key={prod.id}>
                <CustomTableCell 
                  ...
                  sx={{ '&:hover': { cursor: "pointer" }, borderLeftWidth: "0px" }}
                >
                  {capitalizeWords(prod.name)}
                </CustomTableCell>
                <CustomTableCell 
                  onClick={() => handleOpenDetail(prod)} 
                  sx={{ textAlign: 'center', width: "15%", '&:hover': { cursor: "pointer" } }}
                >
                    {typeTransaction[prod.type]}
                </CustomTableCell>
